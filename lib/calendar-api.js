...
export async function findCalendar() {
  ...
  // First check for existing calendar
  const response = await fetch("https://www.googleapis.com/calendar/v3/users/me/calendarList", {
    headers: { "Authorization":  `Bearer ${session?.accessToken}` }
  })
  const calendars = await response.json()

  const calendarName = "Periods of " + session?.user?.name
  const periodsCalendar = calendars.items.find(calendar => calendar.summary === calendarName)
  
  if (periodsCalendar) {
    return periodsCalendar
  }

  // If no calendar exists, create one and verify it was created
  const newCalendar = await createCalendar()
  
  // Double-check to make sure another request didn't create the calendar
  const verifyResponse = await fetch("https://www.googleapis.com/calendar/v3/users/me/calendarList", {
    headers: { "Authorization":  `Bearer ${session?.accessToken}` }
  })
  const updatedCalendars = await verifyResponse.json()
  
  // Return the first calendar that matches our name
  return updatedCalendars.items.find(calendar => calendar.summary === calendarName) || newCalendar
}
...
export async function createRecurringEvents(calendarId, startDate, cycleLength, numberOfCycles = 6) {
  ...
  const existingData = await getRecurringEventsData(calendarId)
  console.log('Existing data:', existingData)
  
  // Use existing values if available
  let description = existingData?.winterDescription
  const previousStartDate = existingData?.startDate
  const previousCycleLength = existingData?.cycleLength

  console.log('Using values:', { description, previousStartDate, previousCycleLength })

  // Calculate the real cycle length based on the difference between startDate and previousStartDate
  if (previousStartDate) {
    const realCycleLength = Math.abs(new Date(startDate) - new Date(previousStartDate)) / (24 * 60 * 60 * 1000); // Convert milliseconds to days
    console.log(`Real cycle length: ${realCycleLength} days`);

    description = description ? 
    `${description}\n${previousStartDate}, ${realCycleLength} days` : 
    `${previousStartDate}, ${realCycleLength} days`
  }
...
export async function getRecurringEventsData(calendarId) {
  try {
    ...
    return {
      ...
      winterDescription: periodEvent.description || null
    }
  } catch (error) {
